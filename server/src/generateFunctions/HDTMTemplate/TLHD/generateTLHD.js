...
const generateTLHD = async (sell_com, buy_com, contract_info, file_name) => {
  ...
  const rule1 = new Paragraph({
    ...
    children: [
      ...
      new TextRun({
        children: [
          new Tab(),
          `Bên A cung cấp dịch vụ thuê máy móc và vận chuyển cho bên B theo đúng yêu cầu.`,
        ],
        size: 24,
        break: 1,
      }),
  ...
  const para6 = new Paragraph({
    ...
    children: [
      ...
      new TextRun({
        children: [
          ` ký ngày ...... / ...... /${contract_info?.year} giữa ${sell_com?.lower_case_name} với ${buy_com?.lower_case_name} V/v: thuê khoán dịch vụ `,
        ],
        italics: true,
        size: 24,
      }),
