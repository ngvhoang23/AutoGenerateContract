...
const handleReadingHDMB = async (file_path) => {
  return workbook.xlsx
    ...
    .then(function () {
      ...
      while (isRunning) {
        ...
        if (isBeforeTaxRow(worksheet, rowNum)) {
          invoice.total_before_tax = worksheet.getCell(`F${rowNum}`)?.value;
          rowNum++;
          continue;
        }
        if (isAfterTaxRow(worksheet, rowNum)) {
...
const isBeforeTaxRow = (worksheet, rowNum) => {
  if (
    worksheet.getCell(`E${rowNum}`)?.value === "before_tax" &&
...
const isAfterTaxRow = (worksheet, rowNum) => {
  if (
...
handleReadingHDMB(String.raw`C:\Users\Admin\Desktop\hdmb_template.xlsx`);

module.exports = { handleReadingHDMB };
