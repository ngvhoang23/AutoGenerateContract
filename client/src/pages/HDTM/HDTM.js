...
import styles from "./HDTM.module.scss";
import Companies from "../../components/Companies/Companies";
import HDMBContractDetail from "../../components/HDMBContractDetail/HDMBContractDetail";
import { useContext, useEffect, useState } from "react";
import { CurrentPageContext } from "../../Context/CurrentPageContext";
import axios from "axios";
import HDTMContractDetail from "../../components/HDTMContractDetail/HDTMContractDetail";
...
function HDTM() {
  ...
  const [contractData, setContractData] = useState({
    ...
    regarding: undefined,
    excel_file_dir: String.raw`C:\Users\Admin\Desktop\hdtm_template.xlsx`,
  ...
  useEffect(() => {
    setContractData((prev) => {
      return {
        ...prev,
        code: `01/HĐDV/${getFirstLetter(sellCom?.short_name)}-${getFirstLetter(
          buyCom?.short_name,
        )}`,
      };
    });
  ...
  const handleSubmit = () => {
    ...
    if (messages.length > 0) {
      if (
        ...
        axios
          .post(`http://localhost:5000/generate-hdtm-contracts`, {
            sell_com: sellCom,
            buy_com: buyCom,
